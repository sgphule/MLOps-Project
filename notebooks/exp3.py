...
mlflow.set_experiment(CONFIG["experiment_name2"])
...
def preprocess_text(text):
    ...
    text = " ".join([lemmatizer.lemmatize(word) for word in text.split() if
                     word not in stop_words])  # Lemmatization & stopwords removal
...
def train_and_log_model(X_train, X_test, y_train, y_test, vectorizer):
    """Trains a xgboost model with GridSearch and logs results to MLflow."""
    ...
    with mlflow.start_run():
        ...
        for params, mean_score, std_score in zip(grid_search.cv_results_["params"],
            ...
            with mlflow.start_run(run_name=f"LR with params: {params}", nested=True):
                ...
                logging.info(f"Params: {params} | Accuracy: {metrics['accuracy']:.4f} | F1: {metrics['f1_score']:.4f}")
